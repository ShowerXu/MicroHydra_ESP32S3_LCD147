...
from launcher import viewers
from lib.hydra import beeper, popup, runtime
...
def open_file(file):
    """Open a file with the relevant built-in handler."""
    cwd = os.getcwd()
    if not cwd.endswith("/"):
        cwd += "/"
    filepath = cwd + file
    ...
    if filetype == "wav":
        viewers.open_wav(filepath)
        return
    if filetype == "bmp":
        viewers.open_bmp(filepath)
        return
    if filetype in ("jpg", "jpeg"):
        viewers.open_unsupported(filepath, "JPEG")
        return
    if filetype == "png":
        viewers.open_unsupported(filepath, "PNG")
        return
    ...
    runtime.run_handler(handler, filepath)
...
def run():
    """Run Files as an in-process launcher handler."""
