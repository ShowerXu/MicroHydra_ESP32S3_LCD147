...
from lib.hydra import beeper, loader, runtime, statusbar
...
_MH_DISPLAY_BACKLIGHT = const(48)
...
def launch_app(app_path):
    """Launch a user app by rebooting, or run a built-in handler in-process."""
    print(f"Launching '{app_path}'...")
    if app_path.startswith("/apps") or app_path.startswith("/sd/apps"):
        if app_path.endswith(".cli.py"):
            loader.launch_app(APP_PATHS['Terminal'], f"${app_path}")
        loader.launch_app(app_path)
    else:
        runtime.run_handler(app_path)


def launch_user_app(app_path):
    """Reboot into a user app."""
    print(f"Launching user app '{app_path}'...")
    if app_path.endswith(".cli.py"):
        loader.launch_app(APP_PATHS['Terminal'], f"${app_path}")
    loader.launch_app(app_path)
...
def main_loop():
    ...
    global DISPLAY  # <-- 必须加上这行！声明我们要修改全局的 DISPLAY
    ...
    # 写入显示
    DISPLAY.show()
    #time.sleep(2) # 暂停2秒，方便你观察屏幕
    # --- 插入测试代码结束 ---
    print("test lcd 2S end.")
    ...
    while True:
        ...
        if new_keys:
            ...
            elif "DOWN" in new_keys:  # left arrow
                PREV_SELECTOR_INDEX = APP_SELECTOR_INDEX
                APP_SELECTOR_INDEX = (APP_SELECTOR_INDEX - 1) % len(APP_NAMES)
            ...
            if "G0" in new_keys or "ENT" in new_keys:
                ...
                if APP_NAMES[APP_SELECTOR_INDEX] == "UI Sound":
                    ...
                    CONFIG.save()
                    selected_path = APP_PATHS[APP_NAMES[APP_SELECTOR_INDEX]]

                    if selected_path.startswith("/apps") or selected_path.startswith("/sd/apps"):
                        # shut off the display before rebooting into untrusted user apps
                        DISPLAY.fill(0)
