...
class I2SSound:
	...
	def __init__(
		...
		self._last_tick = 0
		self._start_output(rate=rate)

	def __del__(self):
		self.deinit()

	def _make_output(self, *, rate, ibuf=None, fmt=I2S.MONO):
		"""Create an I2S TX object, using MCLK when supported by the firmware."""
		kwargs = {
			"sck": Pin(self._sck),
			"ws": Pin(self._ws),
			"sd": Pin(self._sd),
			...
			"format": fmt,
			"rate": rate,
			"ibuf": self._buf_size if ibuf is None else ibuf,
		}
		if self._mck is not None:
			kwargs["mck"] = Pin(self._mck)
		try:
			return I2S(self._i2s_id, **kwargs)
		except TypeError:
			kwargs.pop("mck", None)
			return I2S(self._i2s_id, **kwargs)

	def _start_output(self, *, rate=None):
		"""Start the IRQ-driven mixer output."""
		if rate is not None:
			self._rate = rate
		self._output = self._make_output(rate=self._rate)
